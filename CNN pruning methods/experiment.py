...
def Experiment(dataset_name, dataset_dir, folder_name=None):
    ...
    for i in range(info['num_folds']):
        ...
        model = EvoPruningModel(traingen, testgen, info,
                                final_sparsity=[0.9, 0.9])
        ...
        saver.save_json({'train': res, 'test': test_res},
                        f"notPruned-Fold_{i}.json", path=f'./summary/{folder_name}/')
        ...
        resw = PruningExp(model, 'WEIGHT', pruning_percents,
                          train_time=training_time)
        # model.save_raw_model(f"./summary/{folder_name}/models/weightPruned-Fold_{i}.h5")

        resnf = PruningExp(model, 'NEURON', pruning_percents,
                           'FIRST', train_time=training_time)
        # model.save_raw_model(f"./summary/{folder_name}/models/neuronPrunedFirst-Fold_{i}.h5")

        resns = PruningExp(model, 'NEURON', pruning_percents,
                           'SECOND', train_time=training_time)
        ...
        saver.save_json(
            res, f"WeightPruning-Fold_{i}.json", f"./summary/{folder_name}/")

        all_results.append(res)

    saver.results_mean(
        all_results, f"{dataset_name}_results.json", f"./summary/{folder_name}/")
