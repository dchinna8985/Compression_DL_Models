...
if type_model == 0:  # neuronas
    ...
    fitness = Fitness(first_layer=longitud_cromosoma, second_layer=siguiente_capa,
                      data=datos, layer=layer, optimType=optimization_type)
    test = BGA(pop_shape=(tam_pop, longitud_cromosoma), method=fitness, p_m=p_m, max_evals=max_evals,
    ...
    fitness = Fitness(first_layer=longitud_cromosoma, second_layer=siguiente_capa,
                      data=datos, layer=layer, optimType=optimization_type)
    test = BGA(pop_shape=(tam_pop, longitud_cromosoma*siguiente_capa), method=fitness, p_m=p_m, max_evals=max_evals,
               early_stop_rounds=None, verbose=None, maximum=True, cadena=a_guardar, select=select, crossover=crossover, init=init)
