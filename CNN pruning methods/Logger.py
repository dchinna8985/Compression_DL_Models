...
class Logger(object):
    ...
    def DebugWarning(msg: str):
        print(
            f"(EvoPruning) {Types.WARNING.value}Warning: {msg}{Types.ENDC.value}")

    @staticmethod
    def DebugError(msg: str):
        raise Exception(
            f"(EvoPruning) {Types.ERROR.value}Error: {msg}{Types.ENDC.value}")

    @staticmethod
    def DebugSuccess(msg: str):
        print(
            f"(EvoPruning) {Types.SUCCESS.value}Success: {msg}{Types.ENDC.value}")
