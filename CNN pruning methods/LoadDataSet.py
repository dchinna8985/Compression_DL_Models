...
def get_dataset_info(key, dataset_dir):
    ...
    if n_folds > 1:
        train_paths = [
            f"{dataset_info['train_path']}partition{i}" for i in range(n_folds)]
        test_paths = [
            f"{dataset_info['test_path']}partition{i}" for i in range(n_folds)]
...
def LoadData(path_to_train: str, path_to_test: str,
    ...
    path_to_train = path_to_train + \
        '/' if path_to_train[-1] != '/' else path_to_train
    path_to_test = path_to_test + \
        '/' if path_to_test[-1] != '/' else path_to_test
    if info is not None:
        try:
            len_train = info['len_train']
        except KeyError:
            len_train = len(
                glob(f"{path_to_train}*/*.{file_extension}", recursive=True))
        try:
            len_test = info['len_test']
        except KeyError:
            len_test = len(
                glob(f"{path_to_test}*/*.{file_extension}", recursive=True))
    else:
        len_train = len(
            glob(f"{path_to_train}*/*.{file_extension}", recursive=True))
        len_test = len(
    ...
    info = {'len_train': len_train,
            'len_test': len_test, 'num_classes': classes}
